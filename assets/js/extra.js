...
function notify(element) {
    ...
    var msg = (status == "Pendiente") ? ("?text=Estimado%20cliente%2C%20le%20informamos%20que%20hemos%20recibido%20su%20_" + brand + "%20*" + model + "*_%20para%20darle%20servicio%20por%20*" + fail + "*.%0A%0A%C2%A1Gracias%20por%20su%20preferencia!%20_Kalicel%3A%20*Nosotros%20reparamos*_") : (status == "Listo") ? ("?text=Estimado%20cliente%2C%20le%20informamos%20que%20su%20dispositivo%20_" + brand + "%20*" + model + "*_%2C%20el%20cual%20nos%20trajo%20por%20*" + fail + "*%20est%C3%A1%20listo%2C%20y%20puede%20pasar%20a%20recogerlo.%0A%0A%C2%A1Gracias%20por%20su%20preferencia!%20_Kalicel%3A%20*Nosotros%C2%A0reparamos*_%20") : ("?text=Estimado%20cliente%2C%20le%20confirmarmos%20que%20su%20dispositivo%20_" + brand + "%20*" + model + "*_%2C%20el%20cual%20nos%20trajo%20por%20*" + fail + "*%20fue%20entregado.%0A%0A%C2%A1Gracias%20por%20su%20preferencia!%20_Kalicel%3A%20*Nosotros%C2%A0reparamos*_");
...
function auto_notify(id) {
    ...
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].querySelectorAll('td')[0].textContent == id) {
            rows[i].querySelectorAll('td')[2].querySelectorAll('button')[1].click();
            break;
        }
    }
}